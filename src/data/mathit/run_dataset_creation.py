...
from .utils import code_unpickler, code_pickler
import copyreg
import types
...
# def dsz_sin(x):
#     res = np.sin(x)
#     mask = np.logical_or(x > np.pi * 4, x < -np.pi * 4)
#     res[mask] = np.nan
#     if np.any(mask):
#         print("dsz_sin fuck!!")
#     return res
...
class Pipepile:
    ...
    def create_lambda(self, i):
        ...
        # assert "x_2" in variables

        # print(type(variables))
        # print(len(variables), variables)
        # print("x_2" in variables)
...
def creator(number_of_equations,eq_per_block, debug):
    ...
    if not debug:
        folder_path = Path(f"mathit_data/data/raw_datasets/{number_of_equations}") 
    else:
        folder_path = Path(f"mathit_data/data/raw_datasets/debug/{number_of_equations}")
    ...
    _dataset = dclasses.DatasetDetails(
                               config=config_dict, 
    ...
    t_hf.create_dataset("other", data=np.void(pickle.dumps(_dataset)))
    t_hf.close()

import click
import numpy as np
from .utils import load_eq, load_metadata_hdf5
from .dataset.data_utils import evaluate_fun
import pandas as pd
from collections import defaultdict
from .dataset.data_utils import create_uniform_support, sample_symbolic_constants, evaluate_fun, return_dict_metadata_dummy_constant
#from benchmark import return_order_variables
from torch.distributions.uniform import Uniform
import torch
from .dataset import dclasses
...
def dsz_sin(x):
    ...
    res = np.sin(x)
    mask = np.logical_or(x > math.pi * 4, x < -math.pi * 4)
    res[mask] = math.nan
    # if np.any(mask):
    #     print("dsz_sin fuck!!")
    return res
...
def evaluate_validation_set(validation_eqs: pd.DataFrame, support) -> set:
    ...
    for _, row in validation_eqs.iterrows():
        with warnings.catch_warnings():
            ...
            curr = lambdify(variables,row["eq"], modules=[{'sin': dsz_sin, 'cos': dsz_cos}, 'numpy'])(*support).numpy().astype('float16')
...
@click.command()
@click.option("--data_path", default="mathit_data/data/raw_datasets/10000000/")
@click.option("--csv_path", default="mathit_data/data/benchmark/nc_old.csv")
@click.option("--debug/--no-debug", default=False)
def main(data_path,csv_path,debug):
    print("Loading metadata", data_path)

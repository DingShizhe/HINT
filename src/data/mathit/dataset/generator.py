...
class Generator(object):
    ...
    def infix_to_sympy(cls, infix, variables, rewrite_functions, no_rewrite=False):
        ...
        if expr.has(sp.I):
            expr = expr.replace(sp.I, 1.0)
        if expr.has(sp.I):
            expr = expr.replace(sp.I, 1.0)
            # assert not expr.has(sp.I)
    ...
    def generate_equation(self, rng):
        ...
        while True:
            ...
            if f_expr.count("exp") > 1 or f_expr.count("sin") > 1 or f_expr.count("cos") > 1:
                continue
            elif len(self.var_symbols) == 2 and (not ("x_2" in f_expr and "x_1" in f_expr) ):         # 2-Dim Function
                continue
            elif len(self.var_symbols) == 3 and (not ("x_3" in f_expr and "x_2" in f_expr and "x_1" in f_expr) ):         # 3-Dim Function
                continue
            elif len(self.var_symbols) == 4 and (not ("x_4" in f_expr and "x_3" in f_expr and "x_2" in f_expr and "x_1" in f_expr) ):         # 4-Dim Function
                continue
            else:
                break
